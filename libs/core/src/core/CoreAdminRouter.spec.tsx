        ...
        <AuthContext.Provider value={authProvider}>
          <Router history={history}>
            <CoreAdminRouter layout={Layout}>
              {() => [
                <Resource key='posts' name='posts' list={() => <span>PostList</span>} />,
                <Resource
                  key='comments'
                  name='comments'
        ...
        <AuthContext.Provider value={authProvider}>
          <Router history={history}>
            <CoreAdminRouter
              ...
              layout={Layout}
              loading={Loading}
              customRoutes={[<Route key='foo' noLayout path='/foo' component={Custom} />]}
            >
              {() => new Promise(() => {})}
            </CoreAdminRouter>
          </Router>
    ...
    const { getByText, queryByText } = renderWithRedux(
      <Router history={history}>
        <CoreAdminRouter
          layout={Layout}
          customRoutes={[
            <Route key='foo' noLayout exact path='/foo' render={() => <div>Foo</div>} />,
            <Route key='bar' exact path='/bar' component={() => <div>Bar</div>} />,
          ]}
