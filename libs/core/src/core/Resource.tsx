...
const ResourceRoutes: FunctionComponent<ResourceProps> = ({
  ...
  const isRegistered = useSelector((state: ReduxState) => !!state.admin.resources[name]);
            ...
            <Route
              ...
              render={routeProps => (
                <WithPermissions
                  component={show}
                  basePath={basePath}
                  id={decodeURIComponent((routeProps.match as ResourceMatch).params.id)}
                  {...routeProps}
                  {...resourceData}
                />
              )}
...
const Resource: FunctionComponent<ResourceProps> = ({ intent = 'route', ...props }) =>
  intent === 'registration' ? (
