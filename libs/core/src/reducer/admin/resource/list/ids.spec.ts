...
describe('ids reducer', () => {
  describe('DELETE', () => {
    it('should remove id from ids on Delete action', () => {
      ...
      const newState = idsReducer(['record1', 'record2', 'record3'], action);
  ...
  describe('CRUD_GET_LIST_SUCCESS', () => {
    it('should replace ids with ids from action', () => {
      const action = {
        ...
        payload: {
          data: [{ id: 'new_record1' }, { id: 'new_record2' }, { id: 'new_record3' }],
        },
      };
      const newState = idsReducer(['record1', 'record2', 'record3'], action);
      expect(newState).toEqual(['new_record1', 'new_record2', 'new_record3']);
    });
  });
  ...
  describe('CRUD_CREATE_SUCCESS', () => {
    it('should add new id at the start of ids', () => {
      ...
      const newState = idsReducer(['record1', 'record2', 'record3'], action);
      expect(newState).toEqual(['new_record', 'record1', 'record2', 'record3']);
    });
  });
});
