...
describe('sanitizeEmptyValues', () => {
  ...
  it('should accept deep object values', () => {
    const obj = { foo: { bar: 1 } };
    expect(sanitizeEmptyValues({ obj: { foo: null, foo2: 2 } }, { obj })).toEqual({
      obj: { foo: { bar: 1 }, foo2: null },
    });
  });
  ...
  it('should accept adding objects in arrays', () => {
    ...
    expect(sanitizeEmptyValues({ obj: [{ foo: 1 }, { foo: 4 }] }, { obj })).toEqual({
      obj: [{ foo: 1, foo2: 2 }, { foo: 3 }, { foo: 4 }],
    });
  });
  it('should accept removing objects in array of objects', () => {
    const obj = [{ foo: 1, foo2: 2 }, { foo: 3 }, { foo: 4 }];
    expect(sanitizeEmptyValues({ obj }, { obj: [{ foo: 1 }, { foo: 4 }] })).toEqual({
      obj: [{ foo: 1, foo2: null }, { foo: 4 }],
    });
  });
