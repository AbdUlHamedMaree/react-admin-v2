...
import { FormContextValue, Record as RaRecord, OnSuccess, OnFailure } from '../types';
...
const FormWithRedirect = ({
  ...
  const finalMutators = useMemo(
    () => (mutators === defaultMutators ? mutators : { ...defaultMutators, ...mutators }),
    [mutators]
  );
  ...
  const submit = values => {
    ...
    if (shouldSanitizeEmptyValues) {
      const sanitizedValues = sanitizeEmptyValues(finalInitialValues, values);
...
export type HandleSubmitWithRedirect = (redirect?: RedirectionSideEffect) => void;
