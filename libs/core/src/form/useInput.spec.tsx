...
const Input: FunctionComponent<{ children: (props: any) => ReactElement } & InputProps> =
  ({ children, ...props }) => {
      ...
      <Form
        ...
        render={({ form }) => {
          ...
          return (
            ...
            >
              {({ id, input }) => {
                return <input type='text' id={id} aria-label='Title' {...input} />;
              }}
            </Input>
          );
        }}
        ...
        render={() => {
          return (
            <Input source='views' resource='posts' defaultValue={99}>
              {({ id, input }) => {
                return <input type='number' id={id} aria-label='Views' {...input} />;
              }}
            </Input>
          );
        }}
  ...
  it('applies the initialValue when input does not have a value', () => {
    const { queryByDisplayValue } = renderWithRedux(
      <FormWithRedirect
        ...
        render={() => {
          return (
            <Input source='title' resource='posts' initialValue='foo'>
              {({ id, input }) => {
                return <input type='text' id={id} aria-label='Title' {...input} />;
              }}
            </Input>
  ...
  it('does not apply the initialValue when input has a value of 0', () => {
    const { queryByDisplayValue } = renderWithRedux(
      <FormWithRedirect
        ...
        render={() => {
          return (
            <Input source='views' resource='posts' initialValue={99}>
              {({ id, input }) => {
                return <input type='number' id={id} aria-label='Views' {...input} />;
              }}
            </Input>
