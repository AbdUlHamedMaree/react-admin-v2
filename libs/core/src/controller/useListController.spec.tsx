...
describe('useListController', () => {
    ...
    it('should be synchronized with ids after delete', async () => {
      ...
      const { dispatch, queryByText } = renderWithRedux(
        <DataProviderContext.Provider value={dataProvider}>
          <ListController {...defaultProps} resource='comments' filter={{ foo: 1 }}>
            {({ data, ids }) => (
              <>
                {ids.map(id => (
  ...
  describe('setFilters', () => {
    let clock;
    const fakeComponent = ({ setFilters, filterValues }) => (
      <TextField
        inputProps={{
          'aria-label': 'search',
        }}
    ...
    it('should remove empty filters', () => {
      ...
      const state = reduxStore.getState();
      expect(state.admin.resources.posts.list.params.filter).toEqual({});
      expect(state.admin.resources.posts.list.params.displayedFilters).toEqual({
        q: true,
      });
    });
    ...
    it('should update data if permanent filters change', () => {
      ...
      const { dispatch, rerender } = renderWithRedux(
        <ListController syncWithLocation {...props} filter={{ foo: 1 }} />,
        {
          admin: {
            resources: {
              posts: {
      ...
      rerender(<ListController syncWithLocation {...props} filter={{ foo: 2 }} />);
  ...
  describe('showFilter', () => {
    it('Does not remove previously shown filter when adding a new one', async () => {
      ...
      const fakeComponent = ({ showFilter, displayedFilters, filterValues }) => {
      ...
      const { getByLabelText } = renderWithRedux(<ListController {...props} />, {
        admin: {
          resources: {
            posts: {
              list: {
