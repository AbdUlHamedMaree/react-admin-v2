...
describe('useEditController', () => {
  ...
  it('should return a save callback when undoable is false', async () => {
    ...
    const call2 = dispatch.mock.calls.find(params => params[0].type === 'RA/CRUD_UPDATE');
  ...
  it('should allow onSuccess to override the default success side effects', async () => {
    ...
    const { dispatch } = renderWithRedux(
      <DataProviderContext.Provider value={dataProvider}>
        <SaveContextProvider value={saveContextValue}>
          <EditController {...defaultProps} undoable={false} onSuccess={onSuccess}>
            {({ save }) => {
              saveCallback = save;
              return null;
            }}
  ...
  it('should allow onFailure to override the default failure side effects', async () => {
    ...
    const { dispatch } = renderWithRedux(
      <DataProviderContext.Provider value={dataProvider}>
        <SaveContextProvider value={saveContextValue}>
          <EditController {...defaultProps} undoable={false} onFailure={onFailure}>
            {({ save }) => {
              saveCallback = save;
              return null;
            }}
  ...
  it('should allow transform to transform the data before save', async () => {
    ...
    renderWithRedux(
      <DataProviderContext.Provider value={dataProvider}>
        <SaveContextProvider value={saveContextValue}>
          <EditController {...defaultProps} undoable={false} transform={transform}>
            {({ save }) => {
              saveCallback = save;
              return null;
            }}
