...
describe('<ReferenceArrayInputController />', () => {
  ...
  it('should set loading to true as long as there are no references fetched and no selected references', () => {
    const children = jest.fn(({ loading }) => <div>{loading.toString()}</div>);
    const { queryByText } = renderWithRedux(
      <Form
        onSubmit={jest.fn()}
        render={() => (
          <ReferenceArrayInputController {...defaultProps} input={{ value: [1, 2] }}>
            {children}
  ...
  it('should set error in case of references fetch error and there are no data found for the references already selected', () => {
    ...
    const { queryByText } = renderWithRedux(
      <Form
        onSubmit={jest.fn()}
        render={() => (
          <ReferenceArrayInputController {...defaultProps} input={{ value: [1] }}>
            {children}
          </ReferenceArrayInputController>
        )}
      />,
  ...
  it('should call crudGetMany on mount if value is set', async () => {
    ...
    const { dispatch } = renderWithRedux(
      <Form
        onSubmit={jest.fn()}
        render={() => (
          <ReferenceArrayInputController {...defaultProps} input={{ value: [5, 6] }}>
            {children}
          </ReferenceArrayInputController>
        )}
      />,
  ...
  it('should only call crudGetMatching when calling setFilter', async () => {
    ...
    const { dispatch, getByLabelText } = renderWithRedux(
      <Form
        onSubmit={jest.fn()}
        render={() => (
          <ReferenceArrayInputController {...defaultProps} input={{ value: [5] }}>
            {children}
          </ReferenceArrayInputController>
        )}
      />,
    ...
    await waitFor(() => {
      expect(
        dispatch.mock.calls.filter(call => call[0].type === CRUD_GET_MATCHING).length
      ).toEqual(2);
      expect(
        dispatch.mock.calls.filter(call => call[0].type === CRUD_GET_MANY).length
      ).toEqual(1);
    });
  });
