...
describe('useMatchingReferences', () => {
  ...
  it('should fetch matchingReferences only on mount', () => {
    ...
    expect(dispatch.mock.calls[0][0].type).toBe('RA/CRUD_GET_MATCHING_ACCUMULATE');
    expect(JSON.parse(dispatch.mock.calls[0][0].meta.accumulateKey)).toEqual({
      resource: 'posts',
  ...
  it('should fetch matchingReferences when the filter prop changes', () => {
    ...
    expect(dispatch.mock.calls[1][0].type).toBe('RA/CRUD_GET_MATCHING_ACCUMULATE');
    expect(JSON.parse(dispatch.mock.calls[1][0].meta.accumulateKey)).toEqual({
      resource: 'posts',
      relatedTo: 'comments@post_id',
      pagination: {
        perPage: 25,
