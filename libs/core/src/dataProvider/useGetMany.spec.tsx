...
const UseGetMany = ({ resource, ids, options = {}, callback = null, ...rest }) => {
...
describe('useGetMany', () => {
  it('should call the dataProvider with a GET_MANY on mount', async () => {
    const dataProvider = {
      getMany: jest.fn(() => Promise.resolve({ data: [{ id: 1, title: 'foo' }] })),
    };
    ...
    expect(dataProvider.getMany.mock.calls[0]).toEqual(['posts', { ids: [1] }]);
  });
  ...
  it('should aggregate multiple queries into a single call', async () => {
    ...
    expect(dispatch).toBeCalledTimes(5);
    expect(dataProvider.getMany).toBeCalledTimes(1);
    expect(dataProvider.getMany.mock.calls[0]).toEqual(['posts', { ids: [1, 2, 3, 4] }]);
  });
  ...
  it('should deduplicate repeated ids', async () => {
    ...
    expect(dispatch).toBeCalledTimes(5);
    expect(dataProvider.getMany).toBeCalledTimes(1);
    expect(dataProvider.getMany.mock.calls[0]).toEqual(['posts', { ids: [1, 2, 3] }]);
  });
  ...
  it('should not aggregate or deduplicate calls for different resources', async () => {
    const dataProvider = {
      getMany: jest
        .fn()
        .mockReturnValueOnce(Promise.resolve({ data: [{ id: 1 }, { id: 2 }, { id: 3 }] }))
        .mockReturnValueOnce(
    ...
    expect(dataProvider.getMany.mock.calls[0]).toEqual(['posts', { ids: [1, 2, 3] }]);
    expect(dataProvider.getMany.mock.calls[1]).toEqual(['comments', { ids: [5, 6, 7] }]);
  });
  ...
  it('should retrieve results from redux state on mount', () => {
    ...
    renderWithRedux(<UseGetMany resource='posts' ids={[1, 2]} callback={hookValue} />, {
      admin: {
        resources: {
  ...
  it('should set the error state when the dataProvider fails', async () => {
    ...
    await waitFor(() => {
      expect(hookValue.mock.calls.pop()[0].error).toEqual(new Error('failed'));
    });
  });
  ...
  it('should execute success side effects on success', async () => {
    ...
    renderWithRedux(
      <DataProviderContext.Provider value={dataProvider}>
        <UseGetMany resource='posts' ids={[1, 2]} options={{ onSuccess: onSuccess1 }} />
        <UseGetMany
          resource='comments'
          ids={[3, 4]}
  ...
  it('should execute success side effects once for each hook call', async () => {
    ...
    renderWithRedux(
      <DataProviderContext.Provider value={dataProvider}>
        ...
        <UseGetMany resource='posts' ids={[2]} options={{ onSuccess }} />
      </DataProviderContext.Provider>
    );
    await waitFor(() => {
  ...
  it('should execute failure side effects on failure', async () => {
    ...
    renderWithRedux(
      <DataProviderContext.Provider value={dataProvider}>
        <UseGetMany resource='posts' ids={[1, 2]} options={{ onFailure }} />
      </DataProviderContext.Provider>
    );
    await waitFor(() => {
  ...
  it('should execute failure side effects once for each hook call', async () => {
    ...
    renderWithRedux(
      <DataProviderContext.Provider value={dataProvider}>
        <UseGetMany resource='posts' ids={[1]} options={{ onFailure }} />
        <UseGetMany resource='posts' ids={[2]} options={{ onFailure }} />
      </DataProviderContext.Provider>
    );
    await waitFor(() => {
