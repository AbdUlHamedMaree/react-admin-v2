...
const UseQueryWithStore = ({
  ...
  const hookValue = useQueryWithStore(query, options, dataSelector, totalSelector);
  ...
  it('should not call the dataProvider if options.enabled is set to false and run when it changes to true', async () => {
    ...
    const { rerender } = renderWithRedux(
      <DataProviderContext.Provider value={dataProvider}>
        <UseQueryWithStore callback={callback} options={{ enabled: false }} />
    ...
    rerender(
      <DataProviderContext.Provider value={dataProvider}>
        <UseQueryWithStore callback={callback} options={{ enabled: true }} />
      </DataProviderContext.Provider>,
      { admin: { resources: { posts: { data: {} } } } }
    );
