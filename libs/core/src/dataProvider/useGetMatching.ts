...
import { Identifier, PaginationPayload, SortPayload, Record, ReduxState } from '../types';
...
const useGetMatching = (
  ...
  const {
    ...
    (state: ReduxState) =>
      get(
        ...
        [resource, 'list', 'cachedRequests', JSON.stringify(payload), 'total'],
        null
      )
  );
  ...
  const possibleReferences = getPossibleReferences(referenceState, possibleValues, []);
