...
describe('Query', () => {
  ...
  it('should return the total prop if available', async () => {
    const dataProvider = {
      mytype: jest.fn(() => Promise.resolve({ data: { foo: 'bar' }, total: 42 })),
    };
  ...
  it('should update the error state after an error response', async () => {
    ...
    const dataProvider = {
      getList: jest.fn(() => Promise.reject({ message: 'provider error' })),
    };
    const Foo = () => (
  ...
  it('should dispatch a new fetch action when updating', () => {
    ...
    act(() => {
      rerender(
        <TestContext>
          {() => (
            <Query type='getList' resource='myresource' payload={mySecondPayload}>
              {() => <div>Hello</div>}
            </Query>
          )}
        </TestContext>
