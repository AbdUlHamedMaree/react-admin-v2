  ...
  describe('options', () => {
    ...
    it('should accept an onSuccess option to execute on success', async () => {
      ...
      const UseGetOneWithOnSuccess = () => {
        ...
        const dataProvider = useDataProvider();
        useEffect(() => {
          dataProvider.getOne('dummy', {}, { onSuccess }).then(res => setData(res.data));
        }, [dataProvider]);
        if (data) return <div data-testid='data'>{JSON.stringify(data)}</div>;
        return <div data-testid='loading'>loading</div>;
      };
      const getOne = jest.fn(() => Promise.resolve({ data: { id: 1, foo: 'bar' } }));
      const dataProvider = { getOne };
      renderWithRedux(
        <DataProviderContext.Provider value={dataProvider}>
    ...
    it('should accept an onFailure option to execute on failure', async () => {
      ...
      const UseGetOneWithOnFailure = () => {
        ...
        const dataProvider = useDataProvider();
        useEffect(() => {
          dataProvider.getOne('dummy', {}, { onFailure }).catch(e => setError(e));
        }, [dataProvider]);
        if (error) return <div data-testid='error'>{error.message}</div>;
        return <div data-testid='loading'>loading</div>;
      };
    ...
    it('should accept an enabled option to block the query until a condition is met', async () => {
      const UseGetOneWithEnabled = () => {
        ...
        if (error) content = <div data-testid='error'>{error.message}</div>;
        if (data) content = <div data-testid='data'>{JSON.stringify(data)}</div>;
        return (
          <div>
            {content}
            <button onClick={() => setIsEnabled(e => !e)}>toggle</button>
          </div>
        );
      };
      const getOne = jest.fn().mockResolvedValue({ data: { id: 1, title: 'foo' } });
      const dataProvider = { getOne };
    ...
    describe('mutationMode', () => {
      ...
      it('should not wait for response to dispatch side effects in undoable mode', async () => {
        const update = jest.fn({
          apply: () => Promise.resolve({ data: { id: 1, updated: true } }),
        });
        const dataProvider = { update };
        const UpdateButton = () => {
          const [updated, setUpdated] = useState(false);
