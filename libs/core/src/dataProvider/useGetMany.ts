...
const queriesToCall: QueriesToCall = {};
...
const useGetMany = (
  ...
  const data = useSelector((state: ReduxState) => selectMany(state, resource, ids));
...
const makeGetManySelector = () =>
  createSelector(
    ...
    (resourceData, ids) =>
      resourceData ? ids.map(id => resourceData[id]) : ids.map(id => undefined)
  );
